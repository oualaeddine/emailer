/**
 * docs/07-ui-design.md §7.12 — Langue : Français uniquement.
 * Unique dictionnaire statique de textes d'interface — pas de bascule de
 * langue, pas de bibliothèque i18n multi-locale.
 */
export const fr = {
    common: {
        save: 'Enregistrer',
        cancel: 'Annuler',
        edit: 'Modifier',
        delete: 'Supprimer',
        loading: 'Chargement…',
        confirm: 'Confirmer',
        actions: 'Actions',
        close: 'Fermer',
    },
    nav: {
        dashboard: 'Tableau de bord',
        users: 'Utilisateurs',
        settings: 'Paramètres',
        pagejaunes: 'Annuaire PageJaunes',
        recipients: 'Destinataires',
        import: 'Importer (CSV/Excel)',
        templates: 'Modèles',
        logout: 'Se déconnecter',
        // docs/42-parallel-execution-plan.md §42.5 (WP-00) — nav labels for
        // not-yet-built modules, added up front so Wave 1-3 agents never
        // touch this file's existing keys (§42.11).
        mailbox: 'Boîte de réception',
        campaigns: 'Campagnes',
        suppression: 'Liste de suppression',
        verification: 'Vérification',
        queues: "Files d'attente",
        reporting: 'Rapports',
        audit: "Journal d'audit",
        openMenu: 'Ouvrir le menu de navigation',
        menuTitle: 'Navigation',
        help: 'Aide',
        groups: {
            messaging: 'Messagerie',
            campaigns: 'Campagnes',
            contacts: 'Contacts',
            administration: 'Administration',
        },
    },
    auth: {
        loginTitle: 'Connexion',
        email: 'Adresse e-mail',
        password: 'Mot de passe',
        rememberMe: 'Se souvenir de moi',
        submit: 'Se connecter',
        invalidCredentials: 'Identifiants incorrects ou compte désactivé.',
        twoFactor: {
            // docs/28-security.md §28.1 — TOTP challenge at login.
            challengeTitle: 'Vérification en deux étapes',
            challengeIntro: 'Saisissez le code à 6 chiffres généré par votre application d’authentification.',
            code: 'Code de vérification',
            useRecovery: 'Utiliser un code de récupération',
            useAuthenticator: 'Utiliser l’application d’authentification',
            recoveryCode: 'Code de récupération',
            recoveryIntro: 'Saisissez l’un de vos codes de récupération à usage unique.',
            verify: 'Vérifier',
            // Enrolment.
            setupTitle: 'Configurer la double authentification',
            setupIntro:
                'La double authentification est obligatoire. Scannez ce QR code avec votre application d’authentification (Google Authenticator, Microsoft Authenticator, Authy…), puis saisissez le code généré pour confirmer.',
            manualKey: 'Ou saisissez cette clé manuellement :',
            confirm: 'Activer',
            recoveryCodesTitle: 'Codes de récupération',
            recoveryCodesIntro:
                'Conservez ces codes à usage unique en lieu sûr. Ils permettent de vous connecter si vous perdez l’accès à votre application d’authentification. Ils ne seront plus affichés.',
            recoveryCodesContinue: 'J’ai enregistré mes codes — continuer',
        },
    },
    dashboard: {
        welcome: 'Bienvenue',
        placeholder: 'Le tableau de bord complet sera disponible au fur et à mesure de l’implémentation des modules.',
        // docs/09-dashboard.md §9.2 — widget labels (WP-33).
        sendVolumeTitle: "Volume d'envoi",
        sendVolumeLastDays: 'derniers jours',
        deliverabilityTitle: 'Taux de délivrabilité',
        deliverabilitySent: 'Envoyés',
        deliverabilityDelivered: 'Délivrés',
        quotaUsageTitle: 'Utilisation des quotas SMTP',
        quotaUsageEmpty: 'Aucun compte SMTP actif.',
        quotaUsageNoLimit: 'Sans limite',
        recentCampaignsTitle: 'Campagnes récentes',
        recentCampaignsEmpty: 'Aucune campagne pour le moment.',
    },
    users: {
        title: 'Utilisateurs',
        newUser: 'Nouvel utilisateur',
        name: 'Nom',
        email: 'Adresse e-mail',
        role: 'Rôle',
        status: 'Statut',
        active: 'Actif',
        inactive: 'Inactif',
        lastLogin: 'Dernière connexion',
        never: 'Jamais',
        createTitle: 'Créer un utilisateur',
        editTitle: 'Modifier un utilisateur',
        password: 'Mot de passe',
    },
    roles: {
        administrator: 'Administrateur',
        marketing_manager: 'Responsable marketing',
        marketing_operator: 'Opérateur marketing',
        viewer: 'Lecteur',
    },
    settings: {
        title: 'Identité visuelle',
        organizationName: "Nom de l'organisation",
        brandColor: 'Couleur de marque',
        defaultTheme: 'Thème par défaut',
        light: 'Clair',
        dark: 'Sombre',
        system: 'Système',
        saved: 'Paramètres enregistrés.',
    },
    pagejaunes: {
        title: 'Annuaire PageJaunes',
        searchPlaceholder: 'Rechercher une entreprise par nom ou code…',
        search: 'Rechercher',
        noEmail: "Pas d'email disponible",
        emails: 'E-mails',
        wilaya: 'Wilaya',
        locality: 'Localité',
        noResults: 'Aucun résultat.',
        emptyQuery: 'Saisissez un terme de recherche pour commencer.',
    },
    recipients: {
        title: 'Destinataires',
        newRecipient: 'Nouveau destinataire',
        email: 'Adresse e-mail',
        firstName: 'Prénom',
        lastName: 'Nom',
        companyName: 'Entreprise',
        source: 'Source',
        status: 'Statut',
        tags: 'Étiquettes',
        createTitle: 'Créer un destinataire',
        active: 'Actif',
        suppressed: 'Supprimé',
        invalid: 'Invalide',
        sourceManual: 'Manuel',
        sourceCsvImport: 'Import CSV',
        sourceExcelImport: 'Import Excel',
        sourcePagejaunes: 'PageJaunes',
        sourceInternalContact: 'Contact interne',
        searchPlaceholder: 'Rechercher par nom, e-mail ou entreprise…',
        alreadyExists: 'Un destinataire avec cette adresse e-mail existe déjà.',
        emptyTitle: 'Aucun destinataire trouvé',
        emptyBody: 'Essayez une autre recherche ou ajoutez un nouveau destinataire.',
    },
    imports: {
        title: 'Importer des destinataires',
        chooseFile: 'Choisir un fichier',
        upload: 'Téléverser',
        stepMapping: 'Correspondance des colonnes',
        stepReview: 'Vérification',
        mapEmail: 'Adresse e-mail',
        mapFirstName: 'Prénom',
        mapLastName: 'Nom',
        mapCompanyName: 'Entreprise',
        selectColumn: 'Sélectionner une colonne…',
        none: 'Aucune',
        continue: 'Continuer',
        totalRows: 'Lignes totales',
        validRows: 'Valides',
        invalidRows: 'Invalides',
        duplicateRows: 'Doublons',
        commit: 'Importer les lignes valides',
        summary: 'Résumé de l’import',
        imported: 'Destinataires importés',
        noFileChosen: 'Aucun fichier sélectionné',
        acceptedFormats: 'Formats acceptés : CSV, TXT, XLSX',
    },
    composer: {
        title: 'Nouvel e-mail',
        subject: 'Objet',
        richText: 'Texte enrichi',
        htmlSource: 'Source HTML',
        signature: 'Signature',
        noSignature: 'Aucune',
        saveVersion: 'Enregistrer une version',
        saveVersionSuccess: 'Version %d enregistrée avec succès.',
        saveVersionSuccessDetail: 'La version %d a été archivée dans l’historique des versions.',
        saveVersionError: "Impossible d'enregistrer la version.",
        savingVersion: 'Enregistrement de la version…',
        versionHistory: 'Historique des versions',
        restore: 'Restaurer',
        savedAt: 'Enregistré à',
        saving: 'Enregistrement…',
        previewDesktop: 'Bureau',
        previewTablet: 'Tablette',
        previewMobile: 'Mobile',
    },
    templates: {
        title: 'Modèles',
        newTemplate: 'Nouveau modèle',
        name: 'Nom',
        category: 'Catégorie',
        usage: "Utilisé dans %d brouillon(s)",
        archive: 'Archiver',
        delete: 'Supprimer',
        archived: 'Archivé',
        createTitle: 'Créer un modèle',
        cannotDelete: "Ce modèle est utilisé et ne peut pas être supprimé ; archivez-le à la place.",
        emptyTitle: 'Aucun modèle',
        emptyBody: 'Créez votre premier modèle pour le réutiliser dans vos campagnes et vos e-mails.',
    },
    smtp: {
        title: 'Comptes SMTP',
        newAccount: 'Nouveau compte SMTP',
        name: 'Nom',
        provider: 'Fournisseur',
        host: 'Hôte',
        port: 'Port',
        encryption: 'Chiffrement',
        username: "Nom d'utilisateur",
        password: 'Mot de passe',
        fromEmail: 'Adresse d’expédition',
        dailyQuota: 'Quota quotidien',
        health: 'État',
        healthy: 'Sain',
        degraded: 'Dégradé',
        unhealthy: 'Défaillant',
        disabled: 'Désactivé',
        test: 'Tester',
        testSuccess: 'Connexion réussie',
        testFailure: 'Échec de la connexion',
        testDetails: 'Détails du test',
        testing: 'Test en cours…',
        testResultTitle: 'Résultat du test SMTP',
        testSuccessMessage: 'La connexion au serveur SMTP a été établie avec succès.',
        testFailureMessage: 'Impossible d’établir la connexion avec le serveur SMTP.',
        rawResponse: 'Réponse brute du serveur / diagnostic',
        copyError: 'Copier les détails',
        copied: 'Copié !',
        sslPortTip: 'Conseil de configuration : Vous tentez une connexion SSL directe sur le port 587. Le port 587 requiert généralement le chiffrement TLS (STARTTLS). Pour une connexion SSL directe, utilisez plutôt le port 465.',
        sendTestEmail: 'Envoyer un e-mail de test',
        testEmailAddress: 'Adresse e-mail destinataire',
        testEmailPlaceholder: 'destinataire@example.com',
        testEmailSendButton: 'Envoyer le test',
        testEmailSending: 'Envoi en cours…',
        testEmailSuccess: 'E-mail de test envoyé avec succès.',
        testEmailFailure: 'Échec de l’envoi de l’e-mail de test.',
        retryTest: 'Re-tester',
        viewError: 'Échec (voir l’erreur)',
        viewSuccess: 'Succès (voir détails)',
        testBeforeSave: 'Tester la connexion',
        testBeforeSaveSuccess: 'Connexion réussie ! Vos paramètres SMTP sont fonctionnels. Vous pouvez enregistrer le compte.',
        testBeforeSaveFillRequired: 'Veuillez renseigner l’hôte, le port, le chiffrement, les identifiants et l’e-mail d’expédition pour effectuer le test.',
        causeIdentified: 'Cause identifiée',
        recommendedSolution: 'Solution recommandée',
        quickFixAction: 'Appliquer cette correction',
        quickFixApplied: 'Correction appliquée ! Vous pouvez relancer le test.',
        diagnosticSectionTitle: 'Diagnostic et solution',
        createTitle: 'Créer un compte SMTP',
        cannotDelete: 'Ce compte SMTP a déjà été utilisé pour des envois ; désactivez-le à la place.',
    },
    // docs/42-parallel-execution-plan.md §42.5 (WP-00) — additive namespaces
    // for not-yet-built modules (docs 10/15/22/23/24/25/27), added up front
    // per §42.11 so the file is frozen for Wave 1-3 agents. Kept minimal —
    // owning work packages may still add fine-grained keys for their pages.
    mailbox: {
        title: 'Boîte de réception',
        inbox: 'Boîte de réception',
        sent: 'Envoyés',
        drafts: 'Brouillons',
        outbox: "Boîte d'envoi",
        scheduled: 'Planifiés',
        // docs/10-mailbox.md §10.2/10.4/10.7/10.8 (WP-13) — nested additions
        // for the three-pane mailbox page, additive under this existing
        // namespace per docs/42-parallel-execution-plan.md §42.11.
        searchPlaceholder: 'Rechercher un destinataire ou un objet…',
        viewAllToggle: "Voir toute l'équipe",
        retryNow: 'Relancer maintenant',
        loadMore: 'Charger plus',
        noSelection: 'Sélectionnez un message pour afficher les détails',
        timelineQueued: "Mis en file d'attente",
        timelineSent: 'Envoyé',
        timelineDelivered: 'Distribué',
        timelineOpened: 'Ouvert',
        timelineClicked: 'Cliqué',
        timelineBounced: 'Rejeté',
        timelineFailed: 'Échoué',
        opens: 'Ouvertures',
        clicks: 'Clics',
        smtpAccount: 'Compte SMTP',
        lastResponse: 'Dernière réponse du serveur',
        startNewEmail: 'Nouvel e-mail',
        emptyInboxTitle: 'Aucune notification',
        emptyInboxBody:
            'La boîte de réception affiche uniquement les notifications système (rebonds, alertes de vérification, imports terminés) — pas de courrier entrant réel.',
        emptyDraftsTitle: 'Aucun brouillon',
        emptyDraftsBody: 'Aucun brouillon pour le moment — commencez un nouvel e-mail.',
        emptyOutboxTitle: "Rien en attente d'envoi",
        emptyOutboxBody: "La boîte d'envoi est vide.",
        emptyScheduledTitle: 'Rien de planifié',
        emptyScheduledBody: 'Aucun envoi planifié pour le moment.',
        emptySentTitle: 'Aucun message envoyé',
        emptySentBody: "L'historique des envois est vide.",
        // docs/07-ui-design.md §7.12 — internal MessageStatus identifiers
        // never shown raw; mirrors the status label table there.
        statusLabels: {
            queued: "En file d'attente",
            sending: 'Envoi en cours',
            accepted: 'Accepté',
            delivered: 'Distribué',
            opened: 'Ouvert',
            clicked: 'Cliqué',
            soft_bounced: 'Rebond temporaire',
            hard_bounced: 'Rebond définitif',
            failed: 'Échec',
            rejected: 'Rejeté',
            spam_complaint: 'Signalement spam',
            unsubscribed: 'Désabonné',
        },
    },
    campaigns: {
        title: 'Campagnes',
        newCampaign: 'Nouvelle campagne',
        statusDraft: 'Brouillon',
        statusScheduled: 'Planifiée',
        statusSending: 'Envoi en cours',
        statusSent: 'Envoyée',
        statusPaused: 'En pause',
        statusCancelled: 'Annulée',
        // docs/15-campaign-management.md §15.2/§15.6/§15.8 — WP-23 additions,
        // nested under the existing `campaigns` key only.
        name: 'Nom de la campagne',
        status: 'Statut',
        template: 'Modèle',
        selectTemplate: 'Choisir un modèle',
        scheduledAt: 'Planifiée pour',
        sentAt: 'Envoyée le',
        target: 'Audience',
        audienceSize: "Taille de l'audience",
        // Wizard steps (§15.2).
        stepBasics: 'Informations',
        stepTarget: 'Destinataires',
        stepSchedule: 'Envoi',
        stepReview: 'Vérification',
        wizardTitle: 'Nouvelle campagne',
        targetTypeList: 'Liste de destinataires',
        targetTypeSegment: 'Segment',
        targetId: 'Identifiant de la liste/du segment',
        sendModeNow: 'Envoyer immédiatement',
        sendModeScheduled: 'Planifier',
        reviewSummary: 'Vérifiez les informations avant de continuer.',
        saveDraft: 'Enregistrer comme brouillon',
        sendNow: 'Envoyer maintenant',
        schedule: 'Planifier',
        // Row/detail actions (§15.6/§15.7).
        pause: 'Mettre en pause',
        resume: 'Reprendre',
        cancel: 'Annuler la campagne',
        clone: 'Dupliquer',
        confirmCancel: 'Annuler cette campagne ?',
        confirmSendNow: 'Envoyer cette campagne maintenant ?',
        // Detail tabs (§15.8).
        tabOverview: 'Aperçu',
        tabRecipients: 'Destinataires',
        tabContent: 'Contenu',
        tabAnalytics: 'Analyses',
        quickStats: 'Statistiques',
        noScheduledDate: 'Aucune date planifiée',
        notSentYet: 'Pas encore envoyée',
        noContentPreview: 'Aucun aperçu disponible.',
        recipientStatus: 'Statut du message',
        analyticsSent: 'Envoyés',
        analyticsDelivered: 'Délivrés',
        analyticsOpened: 'Ouverts',
        analyticsClicked: 'Cliqués',
        analyticsBounced: 'Rejetés',
        analyticsComplained: 'Plaintes',
    },
    suppression: {
        title: 'Liste de suppression',
        newEntry: 'Ajouter une adresse',
        email: 'Adresse e-mail',
        reason: 'Motif',
        remove: 'Retirer',
        confirmRemove: 'Retirer cette adresse de la liste de suppression ?',
        // docs/23-suppression-list.md §23.5/§23.7 — WP-14 additions, nested
        // under the existing `suppression` key only.
        createTitle: 'Ajouter une adresse à la liste de suppression',
        notes: 'Notes',
        notesPlaceholder: 'Ex. : demande de retrait via le ticket support #1234',
        dateAdded: "Date d'ajout",
        searchPlaceholder: 'Rechercher par adresse e-mail…',
        allReasons: 'Tous les motifs',
        noResults: 'Aucune adresse dans la liste de suppression.',
        alreadyExists: 'Cette adresse est déjà présente dans la liste de suppression.',
        confirmRemoveStrong:
            'Cette adresse a été supprimée suite à une plainte pour spam ou un rebond définitif. La retirer présente un risque important pour la délivrabilité. Confirmez-vous ce retrait ?',
        reasons: {
            hard_bounce: 'Rebond définitif',
            spam_complaint: 'Plainte pour spam',
            manual_unsubscribe: 'Désabonnement manuel',
            global_unsubscribe: 'Désabonnement global (tout)',
            invalid_address: 'Adresse invalide',
            manual_block: 'Blocage manuel',
        },
    },
    verification: {
        title: 'Vérification',
        verify: 'Vérifier',
        statusValid: 'Valide',
        statusInvalid: 'Invalide',
        statusRisky: 'Risqué',
        statusUnknown: 'Inconnu',
    },
    queues: {
        title: "Files d'attente",
        transactional: 'Transactionnel',
        campaigns: 'Campagnes',
        imports: 'Imports',
        maintenance: 'Maintenance',
    },
    reporting: {
        title: 'Rapports',
        delivery: 'Rapports de livraison',
        campaignComparison: 'Comparaison des campagnes',
        templatePerformance: 'Performance des modèles',
        smtpPerformance: 'Performance SMTP',
        export: 'Exporter',
        // docs/25-reporting.md — WP-31 additions, nested under the existing
        // `reporting` key only (docs/42-parallel-execution-plan.md §42.11 —
        // the pre-added stub above is filled in by this work package).
        subtitle: 'Vue d’ensemble des envois, de la livraison et de l’engagement.',
        dateFrom: 'Du',
        dateTo: 'Au',
        campaignFilter: 'Campagne',
        allCampaigns: 'Toutes les campagnes',
        smtpAccountFilter: 'Compte SMTP',
        allSmtpAccounts: 'Tous les comptes SMTP',
        apply: 'Appliquer',
        sent: 'Envoyés',
        delivered: 'Délivrés',
        opened: 'Ouverts (uniques)',
        clicked: 'Cliqués (uniques)',
        bounced: 'Rejetés',
        failed: 'Échoués',
        totalOpens: 'Ouvertures (total)',
        totalClicks: 'Clics (total)',
        deliveryRate: 'Taux de livraison',
        openRate: "Taux d'ouverture",
        clickRate: 'Taux de clic',
        bounceRate: 'Taux de rejet',
        byStatus: 'Répartition par statut',
        byCampaign: 'Par campagne',
        byCampaignName: 'Campagne',
        bySmtpAccount: 'Par compte SMTP',
        bySmtpAccountName: 'Compte SMTP',
        healthStatus: 'État',
        noData: 'Aucune donnée pour cette période.',
    },
    audit: {
        title: "Journal d'audit",
        action: 'Action',
        actor: 'Utilisateur',
        target: 'Cible',
        timestamp: 'Date',
        export: 'Exporter',
        // WP-32 (docs/42-parallel-execution-plan.md §42.8) additions —
        // the six keys above predate this work package (Wave-0 nav
        // placeholder); left untouched, new keys appended below.
        userIdFilter: 'ID utilisateur',
        actionFilter: 'Action',
        auditableTypeFilter: "Type d'entité",
        dateFromFilter: 'Date de début',
        dateToFilter: 'Date de fin',
        resetFilters: 'Réinitialiser les filtres',
        viewDetails: 'Voir les détails',
        detailsTitle: "Détails de l'événement",
        oldValues: 'Anciennes valeurs',
        newValues: 'Nouvelles valeurs',
        noValues: 'Aucune valeur enregistrée',
        system: 'Système',
        loadMore: 'Charger plus',
        noResults: 'Aucun résultat',
        close: 'Fermer',
        entityType: "Type d'entité",
        entityId: 'ID entité',
        ipAddress: 'Adresse IP',
    },
    // docs/41-notification-center.md §41.5 — Top Bar bell icon + panel.
    // New top-level namespace (WP-34): unlike the WP-00 nav-labeled modules,
    // the notification bell was never a nav-rail item, so no placeholder
    // key existed here before this addition.
    notifications: {
        title: 'Notifications',
        markAllRead: 'Tout marquer comme lu',
        noNotifications: 'Aucune notification',
        unreadAriaLabel: 'notifications non lues',
        loading: 'Chargement des notifications…',
        loadError: 'Impossible de charger les notifications.',
    },
    // Libellés de l'interface d'aide. Le *contenu* de la documentation est
    // bilingue FR/AR et vit dans resources/docs/ ; seuls ces libellés de
    // l'application restent en français uniquement (docs/07-ui-design.md §7.12).
    help: {
        open: 'Aide',
        pageHelp: 'Aide sur cette page',
        docsCenter: 'Centre de documentation',
        title: 'Documentation',
    },
    pwa: {
        installTitle: "Installer l'application",
        installPrompt: 'Installez PageJaunes Mailer pour un accès direct depuis votre bureau et une utilisation fluide hors-ligne.',
        installButton: 'Installer',
        dismissButton: 'Plus tard',
        installSuccess: 'Application installée avec succès !',
        alreadyInstalled: 'Application installée',
        offlineNotice: 'Vous êtes actuellement hors-ligne.',
    },
} as const;

export type TranslationDictionary = typeof fr;
